...
services:
  whatsupdocker:
    ...
    environment:
      ...
      - WUD_TRIGGER_DOCKER_LOCAL_PRUNE=true   # mise a jour automatique (voir §6)
...
## 6. Mettre à jour automatiquement — le mode retenu

Savoir ne suffit pas : le mode retenu sur cette flotte est la **mise à jour automatique**
de tous les conteneurs surveillés. Une seule clé suffit, et c'est l'exemple de la
documentation officielle du trigger Docker :

```yaml
- WUD_TRIGGER_DOCKER_LOCAL_PRUNE=true
```

À chaque passage de veille, dès qu'une image plus récente est trouvée, WUD télécharge
l'image, arrête le conteneur, le supprime, le recrée à l'identique sur la nouvelle image,
le redémarre, puis supprime l'ancienne image. Observé en production au premier passage :
le conteneur est reparti sans intervention, et l'ancienne image a disparu du disque (bon
point pour un `/var` serré).

Par défaut WUD surveille **tous** les conteneurs de la machine : il n'y a rien à étiqueter
pour être mis à jour. La configuration se lit au démarrage :

```bash
docker logs wud | grep -i 'trigger.docker.local'
# le trigger est enregistré avec sa configuration, puis marqué pour exécution automatique
```

### WUD se met à jour lui-même

Le conteneur WUD est surveillé comme les autres. Pour qu'il ne se tue pas en pleine
opération, WUD 9.x démarre un conteneur jetable **`wud-self-update`** qui fait l'échange :
il recrée le conteneur WUD sur la nouvelle image, attend qu'il soit sain, puis disparaît.
La détection du conteneur à remplacer passe par `/proc/self/mountinfo` ; si elle échoue
(racine Docker exotique), l'identifiant se déclare à la main avec `WUD_CONTAINER_ID`.

### Excepter un conteneur

Deux labels, à poser sur le service concerné :

| label | effet |
| --- | --- |
| `wud.watch=false` | WUD **ignore** le conteneur : ni veille, ni notification |
| `wud.trigger.exclude=docker` | WUD **surveille et notifie**, mais ne met **pas** à jour |

Le second est le réglage à employer pour une base de données ou un service qu'on veut
valider à la main : on garde l'alerte « une version plus récente existe », on garde la main
sur le moment de la bascule.

### Ce que la mise à jour automatique ne fait pas

- **aucune vérification après coup** : si la nouvelle image démarre mais casse
  l'application, c'est la supervision et les journaux qui le diront, pas WUD ;
- **aucun retour arrière** : l'ancienne image est supprimée par `prune` — la retirer
  d'abord si on veut pouvoir revenir en arrière ;
- **aucune notion de pile** : un conteneur créé par `docker compose` est recréé seul, hors
  ordre du fichier compose (§8) ;
- **rien à distance** : seuls les conteneurs des machines où WUD tourne sont mis à jour.
...
- **Le trigger Docker met à jour tout seul — c'est le mode retenu (§6).** Déclarer une clé
  `WUD_TRIGGER_DOCKER_LOCAL_*` enregistre le *trigger* Docker : dès qu'une image plus
  récente est trouvée, WUD **arrête, supprime et recrée le conteneur** avec la nouvelle
  image, puis supprime l'ancienne. Aucune autre clé n'est nécessaire : c'est le réglage
  lui-même qui arme l'automatisme. Observé en production sur un conteneur tiers, au premier
  passage de veille. Pour n'être que *notifié* sur un conteneur donné, garder la veille et
  poser le label `wud.trigger.exclude=docker`.
...
- La mise à jour automatique ne vaut que pour les conteneurs **surveillés** : sur une
  machine où WUD n'est pas installé, ou pour un conteneur étiqueté `wud.watch=false`, rien
  n'est mis à jour. L'automatisme n'existe que là où WUD tourne — une machine Docker sans
  WUD reste à la main.
...
*Note rédigée par l'agent Hermes ; version d'origine dans le dépôt `doc-Hermes`,
`Integration/Supervision-conteneurs-Docker.md`.*
