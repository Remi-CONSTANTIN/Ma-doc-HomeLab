...
Je parle ici de simulation, car les SAN d'entreprises habituels se basent sur des infrastructures bien plus conséquentes, avec des baies dédiées, des switchs et des liaisons fibre.  
...
2. Si c'est la première fois que vous vous rendez dans l'application, il vous sera proposé de créer votre premier stockage. Vous pouvez tout de même vous rendre dans l'onglet dédié si vous le désirez
...
> [!note]
> `Initiator Name` : C'est la machine qui se connecte au SAN et qui consomme le stockage. Ils possèdent tous un nom d'initiateur unique (IQN) du type : `iqn.1993-08.org.debian:01:abcdef`
